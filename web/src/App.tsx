...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Box, BrainCircuit, Download, Eraser, FileDown, FileUp, Gamepad2, Grid3X3, Languages, Pencil, Play, Redo2, RotateCcw, Save, Sparkles, Square, Target, Undo2, UserRound, WandSparkles } from 'lucide-react'
...
export default function App() {
  ...
  const publishedItems = useMemo<PackLevel[]>(() => published.map((item) => ({ id: item.id, name: item.title[language], xsb: item.xsb, difficulty: { score: item.optimalPushes, pushes: item.optimalPushes, moves: 0, dependency: 0, trap: 0, away_pushes: 0, box_switches: 0 } })), [published, language])
  ...
  useEffect(() => {
    fetch('/levels/index.json').then((response) => response.json()).then(async (index: { schemaVersion: number; levels: PublishedLevelMeta[] }) => {
      ...
      const loaded = await Promise.all(index.levels.map(async (meta) => ({ ...meta, xsb: await fetch(meta.file).then((response) => response.text()) })))
      setPublished(loaded)
      if (loaded[0]) { const first = parseLevel(loaded[0].xsb); setLevel(first); setInitialLevel(first) }
    }).catch(() => setPublished([]))
  }, [])
  useEffect(() => {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (workMode !== 'play') return
      const next = move(level, direction)
      if (next) { event.preventDefault(); setLevel(next) }
    }
  ...
  function playMove(direction: string) { const next = move(level, direction); if (next) setLevel(next) }
  function switchWorkMode(mode: 'play' | 'edit') { if (mode === 'play') setInitialLevel(level); setWorkMode(mode); setSolveResult(null) }
  ...
  function load(entry: PackLevel) { const loaded = parseLevel(entry.xsb); setLevel(loaded); setInitialLevel(loaded); setHistory([]); setFuture([]); setWorkMode('play'); setSolveResult(entry.solution ? { status: 'solved', moves: entry.solution, pushes: entry.difficulty.pushes, explored_nodes: 0, elapsed_ms: 0, optimal: false, message: '' } : null); setActiveTab('solve') }
  ...
  return <main className="app-shell">
    <header className="topbar">
      ...
      <div className="top-actions">
        <button className="icon-button" title={t.newLevel} onClick={() => { const fresh = parseLevel(SAMPLE); setLevel(fresh); setInitialLevel(fresh); setHistory([]); setWorkMode('edit') }}><Sparkles size={18} /></button>
    ...
    <section className="workspace">
      <aside className={`tool-rail ${workMode === 'play' ? 'locked' : ''}`} aria-label={t.tools}>
        <div className="section-label">{t.tools}</div>
        {(Object.keys(toolIcons) as Tool[]).map((item) => { const Icon = toolIcons[item]; return <button key={item} disabled={workMode === 'play'} className={`tool-button ${tool === item ? 'selected' : ''}`} title={t[item]} onClick={() => setTool(item)}><Icon size={20} /><span>{t[item]}</span></button> })}
        <div className="divider" />
        <div className="dimension-grid"><label>{t.width}<input disabled={workMode === 'play'} aria-label={t.width} type="number" value={level.width} min="5" max="20" onChange={(e) => resize('width', Number(e.target.value))} /></label><label>{t.height}<input disabled={workMode === 'play'} aria-label={t.height} type="number" value={level.height} min="5" max="20" onChange={(e) => resize('height', Number(e.target.value))} /></label></div>
      ...
      <section className="board-area">
        <div className="board-heading"><div><span className={`status-dot ${state}`} />{state === 'solved' ? t.completed : t.board}</div><div className="mode-switch"><button className={workMode === 'play' ? 'active' : ''} onClick={() => switchWorkMode('play')}><Gamepad2 size={14} />{t.playMode}</button><button className={workMode === 'edit' ? 'active' : ''} onClick={() => switchWorkMode('edit')}><Pencil size={14} />{t.editMode}</button></div><span>{level.width} × {level.height}</span></div>
        <div className="board-wrap"><div className="board" style={{ gridTemplateColumns: `repeat(${level.width}, minmax(0, 1fr))` }}>
          {level.cells.map((cell, index) => { const goal = level.goals.includes(index); const hasBox = level.boxes.includes(index); const hasPlayer = level.player === index; const classes = ['cell', cell === '#' ? 'wall' : 'floor', goal ? 'goal' : '', hasBox ? 'box' : '', hasPlayer ? 'player' : '', workMode].filter(Boolean).join(' '); return <button aria-label={`cell ${index}`} key={index} className={classes} onPointerDown={() => { if (workMode === 'edit') paint(index) }} onPointerEnter={(event) => { if (workMode === 'edit' && event.buttons === 1) paint(index) }}>{hasBox && <span className="crate" />}{hasPlayer && <span className="keeper" />}{goal && <span className="goal-mark" />}</button> })}
        </div></div>
        {workMode === 'play' && <div className="play-controls"><button title={t.restartLevel} onClick={() => setLevel(initialLevel)}><RotateCcw size={18} /></button><div className="dpad"><button title="Up" onClick={() => playMove('U')}><ArrowUp size={19} /></button><button title="Left" onClick={() => playMove('L')}><ArrowLeft size={19} /></button><button title="Down" onClick={() => playMove('D')}><ArrowDown size={19} /></button><button title="Right" onClick={() => playMove('R')}><ArrowRight size={19} /></button></div></div>}
      ...
      <aside className="control-panel">
        ...
        {activeTab === 'library' && <div className="panel-body"><h2>{t.library}</h2><ResultList title={t.published} items={publishedItems} empty={t.noResults} onLoad={load} /><div className="import-box" onClick={() => fileInput.current?.click()}><FileUp size={20} /><span>{t.importPack}</span></div><ResultList title={t.myLevels} items={library} empty={t.emptyLibrary} onLoad={load} /></div>}
      </aside>
    </section>
  </main>
}
...
function ResultList({ title, items, empty, onLoad }: { title: string; items: PackLevel[]; empty: string; onLoad: (entry: PackLevel) => void }) {
  return <section className="result-list"><div className="list-header"><h3>{title}</h3><span>{items.length}</span></div>{items.length === 0 ? <p className="empty-state">{empty}</p> : items.slice(0, 50).map((item, index) => <button key={item.id} className="result-row" onClick={() => onLoad(item)}><span className="rank">{index + 1}</span><span className="mini-map"><b>{item.name}</b><small>{item.xsb.split('\n').map((row) => row.replace(/ /g, '·')).join(' / ')}</small></span><span><b>{item.difficulty.score.toFixed(1)}</b><small>{item.difficulty.pushes} pushes</small></span></button>)}</section>
}
